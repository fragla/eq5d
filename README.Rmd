...
<!-- badges: start -->
[![R build status](https://github.com/fragla/eq5d/workflows/R-CMD-check/badge.svg)](https://github.com/fragla/eq5d/actions)
...
Value sets for EQ-5D-3L are available for many countries and have been produced using the time trade-off (TTO) valuation technique or the visual analogue scale (VAS) valuation technique.  Some countries have TTO and VAS value sets for EQ-5D-3L. Additionally, EQ-5D-3L "reverse crosswalk" value sets based on the [van Hout *et al* (2021)](https://doi.org/10.1016/j.jval.2021.03.009) models as well as those published on the [EuroQol](https://euroqol.org/information-and-support/resources/value-sets/) website that enable EQ-5D-3L data to be mapped to EQ-5D-5L value sets are included. 
...
The EQ-5D-5L "crosswalk" value sets published by [van Hout *et al* (2012)](https://doi.org/10.1016/j.jval.2012.02.008) as well as that for Russia are also included. The crosswalk value sets enable index values to be calculated for EQ-5D-5L data where no value set is available by mapping between the EQ-5D-5L and EQ-5D-3L descriptive systems.

The recently published age and sex conditional based mapping data by the [NICE Decision Support Unit](https://sheffield.ac.uk/nice-dsu/methods-development/mapping-eq-5d-5l-3l) are also now part of the package. These enable age-sex based EQ-5D-3L to EQ-5D-5L and EQ-5D-5L to EQ-5D-3L mappings from dimensions and exact or approximate utility index scores. 
...
![Shiny EQ-5D app main screenshot](man/figures/shiny_app_screenshot_barplot.png)
![Shiny EQ-5D app density plot screenshot](man/figures/shiny_app_screenshot_density.png)
![Shiny EQ-5D app ecdf plot screenshot](man/figures/shiny_app_screenshot_ecdf.png)
![Shiny EQ-5D app radar plot screenshot](man/figures/shiny_app_screenshot_radar.png)
![Shiny EQ-5D app posthoc stats plot screenshot](man/figures/shiny_app_screenshot_posthoc.png)
![Shiny EQ-5D app HSDC plot screenshot](man/figures/shiny_app_screenshot_hsdc.png)
![Shiny EQ-5D app HPG plot screenshot](man/figures/shiny_app_screenshot_hpg.png)
